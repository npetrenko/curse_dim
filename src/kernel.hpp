...
public:
    ...
    void Evolve(const Particle<S1>& from, Particle<S2>* output) {
        this->GetDerived()->Evolve(from, output);
    }

    template <class S1, class S2>
    FloatT GetTransDensity(const Particle<S1>& from, const Particle<S2>& to) {
...
public:
    template <class S1, class S2>
    void EvolveConditionally(const Particle<S1>& from, Particle<S2>* output, size_t condition) {
...
public:
    ...
    template <class S1, class S2>
    void EvolveConditionally(const Particle<S1>& from, Particle<S2>* output, size_t action_number) {
